...
class VerifyController extends Controller
{
    ...
    public function verifyUser($token)
    {
        ...
        if($user->email_verified_at == null && $user->user_role == 4){
            ...
            $user->status = 'A';
        	$user->save();
        	return redirect('https://hithere.co.nz/account/login'); 
        }else if($user->email_verified_at == null && $user->user_role == 3){
            $user->email_verified_at = date('Y-m-d h:i:s');
            $user->status = 'A';
            $user->save();
            return redirect('https://hithere.co.nz/account/login')->with('success','Your e-mail is verified. You can now login.');
        }else{
            return redirect('https://hithere.co.nz/account/login')->with('success','Your e-mail is verified. You can now login.');
